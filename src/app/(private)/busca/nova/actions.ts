...
export async function CreateNewShearch(data: SearchSchemaType, userId: string) {
  const supabase = await createClient();

  const { title, description, goodKeyWord, badKeyWord, states, modality } =
    data;
...
`;
  ...
  const removedLicitacoes = [...existingIds].filter(
    (id) => !licitacoesIds.includes(id)
  );
