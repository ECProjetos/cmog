...
interface BuscaTable<TData, TValue> {
  columns: ColumnDef<TData, TValue>[]; // Define as colunas da tabela
  data: TData[]; // Define os dados da tabela
}

export function BuscaTable<TData, TValue>({
  columns,
  data,
}: BuscaTable<TData, TValue>) {
    ...
    <div className="overflow-x-auto">
      <div className="flex items-center justify-between p-4 ">
        <Input
          ...
          value={
            (table.getColumn("titulo")?.getFilterValue() as string) ?? "" // Define o valor do filtro
          }
          onChange={
            (event) =>
              table.getColumn("titulo")?.setFilterValue(event.target.value) // Define o valor do filtro}
          }
          className="max-w-sm"
        />
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                ...
                >
                  Sem buscas realizadas ainda, cire uma busca nova!
                </TableCell>
              </TableRow>
            )}
          </TableBody>
